...
class GAIAResultsAnalyzer:
    ...
    def calculate_accuracy(self) -> Dict[str, float]:
        ...
        successful = sum(1 for r in self.results if not r.get("agent_error") and not r.get("parsing_error"))
    ...
    def analyze_performance_by_task_type(self) -> Dict[str, Any]:
        ...
        for level in self.df["gaia_level"].unique():
            ...
            correct = sum(
                level_results["prediction"].str.lower().str.strip()
    ...
    def analyze_agent_behavior(self) -> Dict[str, Any]:
        ...
        # Planning analysis
        planning_patterns = {"has_facts": 0, "has_detailed_plan": 0, "avg_facts_length": [], "avg_plan_length": []}
        ...
        return {
            ...
            "reasoning_statistics": {
                ...
                "avg_facts_words": np.mean(planning_patterns["avg_facts_length"])
                if planning_patterns["avg_facts_length"]
                else 0,
                "avg_plan_words_detailed": np.mean(planning_patterns["avg_plan_length"])
                if planning_patterns["avg_plan_length"]
                else 0,
            },
            ...
            "action_statistics": {
                ...
                "success_rate": action_patterns["successful_actions"]
                / (action_patterns["successful_actions"] + action_patterns["failed_actions"])
                if (action_patterns["successful_actions"] + action_patterns["failed_actions"]) > 0
                else 0,
    ...
    def generate_detailed_report(self) -> str:
        ...
        report.append("-" * 40)
        report.append(
            f"Agent Errors: {error_analysis['total_agent_errors']} ({error_analysis['error_rates']['agent_error_rate']:.2%})"
        )
        report.append(
            f"Parsing Errors: {error_analysis['total_parsing_errors']} ({error_analysis['error_rates']['parsing_error_rate']:.2%})"
        )
        report.append(
            f"Iteration Limit Exceeded: {error_analysis['total_iteration_exceeded']} ({error_analysis['error_rates']['iteration_exceeded_rate']:.2%})"
        )
        report.append("")
        ...
        if llm_stats["calls_by_level"]:
            ...
            for level_name, level_stats in sorted(
                llm_stats["calls_by_level"].items(),
                key=lambda x: int(x[0].split("_")[1]) if x[0].split("_")[1].isdigit() else 999,
            ):
                level_num = level_name.replace("level_", "")
                report.append(
                    f"    Level {level_num}: Avg={level_stats['avg_calls']:.1f}, "
        ...
        if code_patterns:
            ...
            report.append(
                f"  Average Code Complexity: {np.mean(code_patterns['code_complexity']):.1f} lines"
                if code_patterns["code_complexity"]
                else "  No code complexity data"
            )
        ...
        if "error" not in timing_analysis:
            ...
            report.append(
                f"Total Runtime: {timing_analysis['total_runtime_seconds']:.1f}s ({timing_analysis['total_runtime_seconds'] / 3600:.1f}h)"
            )
                ...
                for level_name, level_stats in sorted(
                    ...
                    if level_num == "3" and "duration_steps_correlation" in level_stats:
                        ...
                        if "longest_tasks_characteristics" in level_stats:
                            ...
                            report.append(
                                f"      Iteration Exceeded Rate: {longest_chars['iteration_exceeded_rate']:.2%}"
                            )
                            report.append(
                                f"      Average Question Length: {longest_chars['avg_question_length']:.0f} chars"
                            )
                            ...
                            report.append(
                                f"    Strongest correlation: Duration vs {strongest_correlation[0]} ({strongest_correlation[1]:.3f})"
                            )
                            ...
                            if level_stats["iteration_exceeded_rate"] > 0.1:
                                report.append(
                                    "    Frequent iteration limit exceeded indicates complex problem-solving"
                                )
            ...
            if flow_patterns.get("success_rates_by_sequence"):
                ...
                for seq, stats in sorted_sequences[:10]:
                    if stats["total_count"] >= 2:  # Only show sequences with multiple instances
                        report.append(f"  {seq}")
                        report.append(
                            f"    Success Rate: {stats['success_rate']:.1%} ({stats['success_count']}/{stats['total_count']})"
                        )
            report.append("")
        ...
        if consistency_analysis and "error" not in consistency_analysis:
            ...
            report.append(
                f"Identical Predictions: {pred_consistency['identical_predictions']}/{consistency_analysis['common_tasks']} ({pred_consistency['identical_predictions_rate']:.1%})"
            )
            report.append(
                f"Consistent Accuracy: {pred_consistency['consistent_accuracy']}/{consistency_analysis['common_tasks']} ({pred_consistency['consistent_accuracy_rate']:.1%})"
            )
            report.append("")
            ...
            if consistency_analysis.get("run_comparison"):
                ...
                for comparison, stats in consistency_analysis["run_comparison"].items():
                    report.append(
                        f"  {comparison}: {stats['agreement_rate']:.1%} ({stats['agreement_count']}/{stats['total_compared']})"
                    )
                report.append("")
            ...
            report.append(
                f"  Accuracy Variance: Mean={acc_var['mean_variance']:.3f}, Max={acc_var['max_variance']:.3f}"
            )
            if duration_var["mean_variance"] > 0:
                report.append(
                    f"  Duration Variance: Mean={duration_var['mean_variance']:.1f}s², Max={duration_var['max_variance']:.1f}s²"
                )
            report.append(
                f"  Step Count Variance: Mean={step_var['mean_variance']:.2f}, Max={step_var['max_variance']:.2f}"
            )
            report.append("")
            ...
            consistent_tasks_count = sum(
                1 for t in consistency_analysis.get("most_consistent_tasks", []) if t["all_same"]
            )
    ...
    def export_detailed_csv(self, output_file: str = "gaia_detailed_results.csv") -> None:
        ...
        export_df["is_correct"] = (
            export_df["prediction"].str.lower().str.strip() == export_df["true_answer"].str.lower().str.strip()
        )
                ...
                if level == 3 and len(data["durations"]) > 1:
                    try:
                        ...
                        if data["task_details"]:
                            sorted_tasks = sorted(data["task_details"], key=lambda x: x["duration"] or 0, reverse=True)
                            ...
                            avg_longest = {
                                ...
                                "iteration_exceeded_rate": sum(t["iteration_exceeded"] for t in longest_tasks)
                                / len(longest_tasks),
    ...
    def analyze_detailed_step_patterns(self) -> Dict[str, Any]:
        ...
        for result in self.results:
            steps = result.get("intermediate_steps", [])
            is_correct = result.get("prediction", "").lower().strip() == result.get("true_answer", "").lower().strip()
                ...
                if step.get("step_type") == "action":
                    ...
                    for tool_call in tool_calls:
                        if (
                            ...
                            and tool_call.get("function", {}).get("name") == "python_interpreter"
                        ):
    ...
    def analyze_consistency_across_runs(self) -> Dict[str, Any]:
        ...
        sorted_by_consistency = sorted(task_consistency.items(), key=lambda x: x[1]["consistency_score"], reverse=True)
    ...
    def create_visualizations(self, output_dir: str = "./analysis_plots") -> None:
        ...
        if level_distribution and level_distribution["level_distribution"]:
            ...
            valid_distributions = {
                k: v
                for k, v in level_distribution["level_distribution"].items()
        ...
        if "gaia_level" in self.df.columns and (self.df["gaia_level"] != "unknown").any():
            ...
            if not valid_levels_df.empty:
                task_level_accuracy = valid_levels_df.groupby("gaia_level").apply(
                    lambda x: (
                        x["prediction"].str.lower().str.strip() == x["true_answer"].str.lower().str.strip()
                    ).mean()
                )
        ...
        error_counts = {
            ...
            "Successful": (
                ~self.df["agent_error"].notna()
                & ~self.df["parsing_error"].notna()
                & ~self.df["iteration_limit_exceeded"]
            ).sum(),
        }
        ...
        plt.figure(figsize=(10, 6))
        self.df["is_correct"] = (
            self.df["prediction"].str.lower().str.strip() == self.df["true_answer"].str.lower().str.strip()
        )
        ...
        plt.hist(
            [correct_steps, incorrect_steps], bins=20, alpha=0.7, label=["Correct", "Incorrect"], edgecolor="black"
        )
...
def main():
    ...
    parser.add_argument(
        "--output_dir", type=str, default="./output/validation", help="Directory containing JSONL result files"
    )
    parser.add_argument("--pattern", type=str, default="*.jsonl", help="File pattern to match (default: *.jsonl)")
    parser.add_argument("--export_csv", action="store_true", help="Export detailed results to CSV")
    parser.add_argument("--create_plots", action="store_true", help="Create visualization plots")
    parser.add_argument("--plot_dir", type=str, default="./analysis_plots", help="Directory to save plots")
