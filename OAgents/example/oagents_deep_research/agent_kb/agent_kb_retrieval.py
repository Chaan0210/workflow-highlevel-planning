...
class AgenticKnowledgeBase:
    def __init__(self, json_file_paths=None):
        ...
        self.field_components = {
            "query": {"vectorizer": TfidfVectorizer(stop_words="english"), "matrix": None, "workflow_ids": []},
            # 'plan': {
...
class AKB_Manager:
    ...
    def hybrid_search(self, query: str, top_k: int = 5, weights: Dict[str, float] = None) -> List[dict]:
        ...
        # for field in ['query', 'plan', 'search_plan']:
        for field_name in ["query"]:
            for result in self.knowledge_base.field_text_search(query, field_name, top_k * 2):
                score_board[result["workflow_id"]] += weights["text"] * field_weights[field_name] * result["score"]
