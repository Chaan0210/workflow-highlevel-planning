...
class SimpleVectorRetriever:
    ...
    def __init__(
        ...
        self.storage = (
            ...
            if path is None
                ...
                vector_dim=self.embedding_model.get_output_dim(), collection_name="rag_cache", path=path
            )
        )
    ...
    def retrieve(self, query, contents, limit: int = 10, threshold: float = 0.3, embed_batch: int = 50):
        """
        @inputs
            query: str
        ...
        if len(results) == 0:
            return (
                "Not enough relative contents! Maybe we should search more information from other source for the task"
            )
        # rank
