...
from serpapi import BaiduSearch, BingSearch, GoogleSearch, YahooSearch
...
class BaseSearcher:
    ...
    def _to_content(self, query: str, snippets: List):
        ...
        for search_info in snippets:
            ...
            redacted_version = f"{idx}. [{title}]({link})" + f"{date}{source}\n{self._pre_visit(link)}{snippet}"
...
class SerpSearcher(BaseSearcher):
    def __init__(self, engine: str = "google", api_key: str = None, max_results: int = 10):
        super().__init__()
        self.engine = engine
    ...
    def search(self, query: str, filter_year: Optional[int] = None):
        ...
        try:
            ...
            if "error" in data:
                ...
                return (
                    f"Wikipedia API error: {error_info.get('code', 'unknown')} - {error_info.get('info', 'unknown')}"
                )
            ...
            for page_id, page_info in pages.items():
                ...
                result = {"idx": idx, "title": title, "date": "", "snippet": extract, "source": "", "link": page_url}
...
dictionaries, each representing a search result."""
    ...
    def search(self, query: str, filter_year: Optional[int] = None, source: str = "text") -> List[Dict[str, Any]]:
        ...
        if source == "text":
            ...
            if results and len(results) > 0:
                ...
                responses.append(
                    {
                        "error": "No results found in DuckDuckGo search (may be rate limited - try a different search engine)"
                    }
                )
        ...
        """
        responses.append({"tips": additional_text.strip()})
        return responses
...
class SearchTool(Tool):
    ...
    def __init__(self, search_type: str = "google", serp_num: int = 5, reflection: bool = False):
        super().__init__()
